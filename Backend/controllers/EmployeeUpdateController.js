...
const employeeUpdate = async (req, res) => {
    ...
    try {
      let {
        ...
        status,
        password,
        updatedby,  // Replacing createdby with updatedby
        image
      } = req.body;
      ...
      if (!userId || !user_name || !updatedby) {
        return res
          .status(400)
          .json({ message: "userId, user_name, and updatedby are required" });
      }
      ...
      const employeeData = {
        ...
        UpdatedBy: updatedby,  // Using updatedby
        Image: imageBuffer
      };
      ...
      employeeRequest.input("UpdatedBy", sql.NVarChar, employeeData.UpdatedBy);  // Updated by
      ...
      loginRequest.input("UpdatedBy", sql.NVarChar, userLoginData.UpdatedBy);  // Updated by
      if (hashedPassword) {
